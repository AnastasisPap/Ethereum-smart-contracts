...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name(), 'PAP Token');
    assert.equal(await instance.symbol(), 'PAP');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Test Star 1', tokenId1, { from: accounts[0] });
    await instance.createStar('Test Star 2', tokenId2, { from: accounts[1] });
...
it('lets a user transfer a star', async() => {
    ...
    assert.notEqual(await instance.ownerOf(tokenId), accounts[0]);
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    assert.equal('Test Star', starName);
});